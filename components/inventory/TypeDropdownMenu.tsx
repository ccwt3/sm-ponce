...
export function TypeCombobox({ value = "", onValueChange }: TypeComboboxProps) {
  const { productTypes, error, creating, handleAddType, refetch } =
    useProductTypes();
  ...
  async function confirmDeleteType() {
    ...
    try {
      ...
      setDeleteError(
        err instanceof Error ? err.message : "Error al eliminar tipo",
      );
    } finally {
  ...
  return (
    <>
              ...
              <CommandGroup>
                {filteredTypes.map((type) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      aria-label={`Eliminar tipo ${type.tipo_de_producto}`}
                      onMouseDown={(event) => {
                        event.preventDefault();
                        event.stopPropagation();
                      }}
                      onClick={(event) => requestDeleteType(event, type)}
                      className="invisible rounded p-0.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive group-hover:visible"
                    >
      ...
      <ConfirmDeleteDialog
        ...
        entityLabel="tipo"
        itemName={typeToDelete?.tipo_de_producto ?? ""}
